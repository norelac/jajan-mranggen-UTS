...
<div class="container py-5">
    ...
    <div class="mb-4 d-flex flex-wrap gap-2 align-items-center">
        <span class="fw-600 small text-muted me-1"><i class="fas fa-tags me-1"></i>Filter Tag:</span>
        <a href="/produk"
           class="badge rounded-pill px-3 py-2 text-decoration-none <?= empty($activeTag) ? 'text-white' : '' ?>"
           style="background: <?= empty($activeTag) ? 'var(--primary)' : '#f0f0f0' ?>; color: <?= empty($activeTag) ? '#fff' : '#555' ?>; font-size:.82rem;">
            Semua
        </a>
        <?php foreach ($allTags as $tag): ?>
            <?php if (($tag['product_count'] ?? 0) > 0): ?>
            <a href="/produk?tag=<?= esc($tag['slug']) ?>"
               class="badge rounded-pill px-3 py-2 text-decoration-none"
               style="background: <?= ($activeTag ?? '') === $tag['slug'] ? $tag['color'] : $tag['color'] . '22' ?>; color: <?= ($activeTag ?? '') === $tag['slug'] ? '#fff' : $tag['color'] ?>; border: 1px solid <?= $tag['color'] ?>44; font-size:.82rem; transition: all .2s;">
                #<?= esc($tag['name']) ?> <span style="opacity:.75;">(<?= $tag['product_count'] ?>)</span>
            </a>
            <?php endif; ?>
        <?php endforeach; ?>
    </div>
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <div class="card product-card h-100">
                    ...
                    <div class="card-body d-flex flex-column">
                        <span class="badge-category mb-1 d-inline-block"><?= esc($product['category_name']) ?></span>

                        <!-- Product Tags -->
                        <?php
                        $ptags = (new \App\Models\TagModel())->getProductTags($product['id']);
                        foreach (array_slice($ptags, 0, 2) as $pt):
                        ?>
                            <span class="badge rounded-pill me-1 mb-1" style="background:<?= esc($pt['color']) ?>22; color:<?= esc($pt['color']) ?>; font-size:.68rem;">#<?= esc($pt['name']) ?></span>
                        <?php endforeach; ?>

                        <h6 class="card-title fw-600 flex-grow-1 mt-1" style="font-size:.9rem; line-height:1.4;">
                        ...
                        <div class="d-flex gap-2 mt-2">
                            <a href="/produk/<?= esc($product['slug']) ?>" class="btn btn-outline-secondary btn-sm flex-grow-1 rounded-pill" style="font-size:.8rem;">Detail</a>
